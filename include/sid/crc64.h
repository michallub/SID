...
constexpr inline uint64_t CRC64Table[256] = {
...
constexpr uint64_t crc64(const char *data, const size_t size, const size_t stride = 1) noexcept {
	...
	{
		uint8_t byte = data[i];
		uint8_t LUTindex = ((crc >> 56) ^ byte) & 0xFFu;
		crc = (crc << 8u) ^ CRC64Table[LUTindex];
	}

	return crc;
}

constexpr uint64_t crc64WE(const char*data, const size_t size, const size_t stride = 1) noexcept {
	if (data == nullptr || size == 0)
		return 0;
